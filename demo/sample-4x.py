...
def upscale_image(img, idx, pipe, gpu_id):
...
def main():
    ...
    # 加载图像
    low_images = [Image.open(f'../results/demo/1.png').convert('RGB')]
    print('load image')

    # 使用 ThreadPoolExecutor 进行并行处理
    num_gpus = torch.cuda.device_count()
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_gpus) as executor:
        futures = [executor.submit(upscale_image, img, idx, pipe, idx % num_gpus) for idx, img in enumerate(low_images)]
